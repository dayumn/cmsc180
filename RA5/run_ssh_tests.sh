#!/bin/bash
# ============================================================
# LRP05 SSH Automation Script
# CMSC 180 - Distributed MMT over Sockets (Core Affinity)
# ============================================================
...
#   4. Compile first:               gcc -O2 -o lab05_coreaffine lab05_coreaffine.c
...
REMOTE_DIR="/tmp/lrp05"   # Scratch dir on slave PCs
OUTPUT_CSV="results_ssh.csv"
SLAVE_OUTPUT_CSV="results_slaves_ssh.csv"
...
start_slaves() {
    ...
    for (( rank=0; rank<t; rank++ )); do
        ...
        $SSH "${SSH_USER}@${ip}" \
            "cd ${REMOTE_DIR} && echo '${n} ${port} 1' | ./lab05_coreaffine \
...
kill_slaves() {
    ...
    for (( rank=0; rank<t; rank++ )); do
        ...
        if [[ -z "${seen[$ip]}" ]]; then
            seen[$ip]=1
            $SSH "${SSH_USER}@${ip}" "pkill -f lab05_coreaffine 2>/dev/null; true" &
        fi
    done
    wait
}
...
# -------------------------------------------------------
# collect_slave_logs_run <t> <run>
# -------------------------------------------------------
collect_slave_logs_run() {
    local t=$1 run=$2
    local run_dir="slave_logs/run${run}"
    mkdir -p "$run_dir"
    for (( rank=0; rank<t; rank++ )); do
        local ip
        ip=$(get_slave_ip $rank)
        $SCP -q "${SSH_USER}@${ip}:${REMOTE_DIR}/slave_rank${rank}.log" \
            "${run_dir}/rank${rank}_${ip}.log" 2>/dev/null || true
    done
}

# -------------------------------------------------------
# compute_slave_max_from_logs <t> <run>
# -------------------------------------------------------
compute_slave_max_from_logs() {
    local t=$1 run=$2
    local run_dir="slave_logs/run${run}"
    local max_slave=0.000000
    for (( rank=0; rank<t; rank++ )); do
        local ip
        ip=$(get_slave_ip $rank)
        local log_file="${run_dir}/rank${rank}_${ip}.log"
        local slave_time
        if [[ -f "$log_file" ]]; then
...
check_prerequisites() {
    ...
    if [[ ! -x "$BINARY" ]]; then
        error "Binary '$BINARY' not found. Compile first: gcc -O2 -o lab05_coreaffine lab05_coreaffine.c"
        exit 1
    fi
...
main() {
    echo -e "${CYAN}"
    echo "================================================="
    echo "  LRP05 — Distributed MMT via Sockets (SSH)"
    echo "  CMSC 180 | $(date)"
    echo "================================================="
    echo -e "${NC}"
    ...
    echo "n,t,Max of Run 1,Max of Run 2,Max of Run 3,Average" > "$SLAVE_OUTPUT_CSV"
    ...
    for n in "${N_VALUES[@]}"; do
        for t in "${T_VALUES[@]}"; do
            ...
            for (( run=1; run<=RUNS; run++ )); do
                ...
                collect_slave_logs_run "$t" "$run"
                local max_slave
                max_slave=$(compute_slave_max_from_logs "$t" "$run")
                slave_max_times+=("$max_slave")
                slave_sum=$(awk "BEGIN { printf \"%.6f\", $slave_sum + $max_slave }")
            done
            ...
            echo "${n},${t},${slave_max_times[0]},${slave_max_times[1]},${slave_max_times[2]},${slave_avg}" >> "$SLAVE_OUTPUT_CSV"
